...
function TeacherCard({ teacher }) {
  const { firstName, lastName, picture, specialization, courses } = { ...teacher }
  console.log(teacher);
  ...
  return (
    <div className="relative rounded-[1.6rem] mx-auto bg-brand-200 text-brand-900 w-[25.5rem] flex flex-col items-center justify-center">
      ...
      <div className="flex items-center justify-between w-full px-8 mt-[10rem]">
        <p className="font-bold text-[2rem]">أ {firstName + " " + lastName}</p>
      ...
      <div className=" w-full px-8 mt-4">
        {/* Subject and Level */}
        <p className="font-bold text-[2.5rem] mb-4">{specialization}</p>
        <p className="font-bold text-[2rem]">{courses[0]?.title?.replace(/^\d+-\s*/, '') || "الصف الثالث الثانوي"}</p>
      </div>
      <div className="flex items-center flex-col gap-4 w-full justify-between  px-8 py-4 mt-4">
        <Link
          to={`/courses/${courses[0]?.id ? courses[0]?.id : ''}`}
