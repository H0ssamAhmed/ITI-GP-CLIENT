...
const CourseCatalog = () => {
  ...
  return (
    <div className='bg-brand-600/10' style={{ backgroundImage: 'url("../../../assets/backgroundcover.png")' }}>
      <div className='container mx-auto py-40 bg-white'>
        <div
          onClick={() => setShowFiltreDiv(!showFiltreDiv)}
          className='px-4 py-2 start-0  sticky -mt-48 top-[7rem] md:hidden  mx-auto transition-all z-10 b-white w-full cursor-pointer'>
          <AnimatePresence>
            {!showFiltreDiv &&
              <motion.div
        ...
        <div className='relative grid grid-cols-12 gap-8 mt-20 md:mt-0'>
          <motion.div
            initial={{ x: 2000, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ duration: 0.3 }}
            className='h-[800px] hidden ms-8 sticky top-40 overflow-y-scroll md:block col-span-4 lg:col-span-3 mx-auto py-4 border-2 border-brand-400 w-full'>
            <LevelsAndCourses
              subject={subject}
              level={level}
              resetAllLevers={resetAllLevers}
