...
const TeacherLessons = () => {
  ...
  const {
    ...
    isLoading: isTeacherSectionsLoading,
    error: isTeacherSectionsError,
    isPending: isTeacherSectionsPending,
    refetch: refetchRequests,
  } = useQuery({
    queryKey: ["courses"],
    queryFn: FetchTeacherSections, // Ensure this function is defined correctly
  });

  if (isTeacherSectionsLoading) return <Spinner />;
  if (isTeacherSectionsLoading)
    return <ErrorMessage message="فشل تحميل بيانات الكورسات" />;

  const courseData = teacherSections || [];
    ...
    >
      ...
      <td className="flex gap-2">
        ...
        <button
          className="flex items-center justify-center w-10 h-10"
          title="Delete"
          disabled={isTeacherSectionsPending}
          onClick={() => {
